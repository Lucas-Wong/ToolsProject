...
"""
-----------------------------------------------------------
 Name：            ToolsProject/monitorserver
 Purpose：         
...
def mem_info():
    child = ssh_command(service_user, ip, service_password, "cat /proc/meminfo")
...
def vm_stat_info():
    child = ssh_command(service_user, ip, service_password, "vmstat 1 2 | tail -n 1")
    child.expect(pexpect.EOF)
...
def cpu_info():
    child = ssh_command(service_user, ip, service_password, "cat /proc/cpuinfo")
    child.expect(pexpect.EOF)
...
def load_stat():
    child = ssh_command(service_user, ip, service_password, "cat /proc/loadavg")
    child.expect(pexpect.EOF)
...
def ionetwork():
    child = ssh_command(service_user, ip, service_password, "cat /proc/net/dev")
    child.expect(pexpect.EOF)
...
def disk_stat():
    child = ssh_command(service_user, ip, service_password, "df -h")
    child.expect(pexpect.EOF)
    disk = child.before
...
def getComStr():
    child = ssh_command(service_user, ip, service_password, "netstat -tpln")
    child.expect(pexpect.EOF)
    Com = child.before
    print('******************************端口监控*********************************')
    print("*******************时间：", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), "******************")
...
def cpu():
    child = ssh_command(service_user, ip, service_password, 'cat /proc/stat | grep "cpu "')
    child.expect(pexpect.EOF)
    child1 = ssh_command("远程服务器用户名", "172.16.1.215", "远程服务器密码", 'cat /proc/stat | grep "cpu "')
