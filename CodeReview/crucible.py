...
SCRIPT_VERSION = 'cd9734cfa42c09f1490da7f4f8b99d65'
...
class Configuration(object):
    ...
    def fill_from_defaults(self):
        self.url, self.username = self.url or 'http://crucible.ariix.com:8060', self.username or 'lucas.wang'
        return self
    ...
    def fill_from_args(self, args):
        ...
        self.git_format = args.gitformat or self.git_format

        return self

    def validate(self, check_title=False, check_id=False):
        """Checks if all the required server options are set"""
        if self.url and (self.authtoken or (self.username and self.password)) and (not check_id or self.id) and (
            not check_title or self.title):
            return True
        ...
        if not sys.stdin.isatty():
            ...
            if get_id:
                while not self.id:
                    ...
                    self.id = raw_input(
                        'Please specify a project to create the review in or an existing review id to add to%s: ' % last_project_prompt).strip()
            ...
            if get_reviewers and not self.reviewers:
                while True:
                    reviewer = raw_input(
                        'Please specify a reviewer to be added to the review, or press Enter to continue: ').strip()
                    if not reviewer:
                        break
    ...
    def choose_source(self, matching_patch_groups):
        ...
        if len(matching_patch_groups) == 1:
            print('Adding patch to existing one: %s. Use --newpatch to add as a new patch instead.' %
                  matching_patch_groups[0]['displayName'])
...
class ConfigFile:
    ...
    def save(self):
        try:
            ...
            with os.fdopen(os.open(self.userConfigPath, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600),
                           'wb') as configfile:
...
EXAMPLE USAGE:
 ...
 %(executable)s -r repository1
        ...
        self.parser.add_argument('project/review', type=str, nargs='?', default=None,
                                 help='the name of the project to create the review in or the id of a review to update\n\n')
        self.parser.add_argument('@reviewer', type=str, nargs='*', default=[],
                                 help='the usernames of the reviewers to be added')
        ...
        self.parser.add_argument('-n', '--noanchor', action='store_const', const=True,
                                 help='don\'t try to detect the repository to anchor the patch\n\n')
        self.parser.add_argument('-N', '--newpatch', action='store_const', const=True,
                                 help='add as a new patch instead of trying to an existing one\n\n')
        self.parser.add_argument('-e', '--encoding', type=str,
                                 help='the name of the encoding to use, see http://docs.python.org/2/library/codecs.html#standard-encodings\n\n')
        self.parser.add_argument('--patch', action='store_const', const=True,
                                 help='print the patch, instead of creating a review')
        self.parser.add_argument('-g', '--gitformat', action='store_const', const=True,
                                 help='use Git Extended Diff format for Mercurial repository. Flag is ignored for other repository types\n\n')
        self.parser.add_argument('-d', '--debug', action='store_const', const=True,
...
class CrucibleRest:
    ...
    def __build_payload(self, data):
        ...
        encodings = self._guess_encodings()
    ...
    def _guess_encodings(self):
        ...
        if self.configuration.encoding:
            ...
            try:
                ...
                logging.debug("getdefaultlocale raised error, ignoring")

            if default_encoding and default_encoding not in encodings:
                encodings.append(default_encoding)
            if sys.stdin.encoding and sys.stdin.encoding not in encodings:
    ...
    def _request(self, url, data=None, http_handlers={}, use_token=True, custom_headers={}):
        ...
        try:
            response = urllib2.build_opener(HTTPRedirectHandler()).open(request, data=payload,
            ...
            if error.code == 404:
                error_msg = error_body['message'] if isinstance(error_body, dict) and 'message' in error_body \
                    else 'Please check that %s is a Crucible server and the version is at least %s' % (
                self.configuration.url, MIN_CRUCIBLE_VERSION)
                Console.error('Error : %s' % error_msg)
            elif error.code == 401:
                Console.error(
                    'Authorization error: please check that your username and password are valid, and that you have the correct permissions.')
            elif error.code == 403:
                error_msg = error_body['message'] if isinstance(error_body,
                                                                dict) and 'message' in error_body else 'You do not have permission to perform this operation'
                permissions_msg = 'Please contact your administrator if you need access'
                Console.error('Error: %s\n%s' % (error_msg, permissions_msg))
            elif error.code == 500:
                Console.error('Server Error: %s' % error_body['message'] if isinstance(error_body,
                                                                                       dict) and 'message' in error_body else error_body)
            else:
                Console.error('Received an unexpected response %s. Please check that %s is a Crucible server' % (
                error, self.configuration.url))
            sys.exit(1)
    ...
    def check_connection(self):
        ...
        if not self.configuration.authtoken:
            ...
            if not login_success:
                message = login_response['error'] if isinstance(login_response,
                                                                dict) and 'error' in login_response else 'Please check that the username and password provided are correct.'
        ...
        if not server_info['isCrucible']:
            ...
            sys.exit(1)
        logging.debug('Connected successfully to %s - Crucible version %s', self.configuration.url,
                      server_info['version']['releaseNumber'])
    ...
    def add_reviewers(self, review_id, reviewers):
        ...
        for reviewer in reviewers:
            if reviewer:
                reviewer_added, reviewer_msg = self._request("/rest-service/reviews-v1/%s/reviewers" % str(review_id),
                                                             data=reviewer,
                                                             http_handlers={
                                                                 404: lambda http_error, error_body: (False,
                                                                                                      'No user \'%s\' found - not adding as a reviewer' % reviewer),
                                                                 400: lambda http_error, error_body: (False, error_body[
                                                                     'message'] if 'message' in error_body else 'Error adding reviewer %s' % reviewer),
    ...
    def handle_anchor_error(self, http_error, error_body):
        if error_body.get('code') == CrucibleRest.code_anchor_failed:
            Console.error(
                'Error: Failed to anchor the patch to repository %s, please check that the repository is the correct one' % self.configuration.repository)
            sys.exit(1)
        elif error_body.get('code') == CrucibleRest.code_content_too_large or error_body.get(
                'code') == CrucibleRest.code_review_content_too_large:
    ...
    def create_review(self, patch, project):
        ...
        if reviewers_added or project['defaultReviewers']:
            ...
            approve_response = self._request(
                '/rest-service/reviews-v1/%s/transition?action=action:approveReview' % review_id, data=' ',
                http_handlers={
    ...
    def get_review(self, id):
        ...
        try:
            return self._request('/rest-service/reviews-v1/%s' % id,
                                 http_handlers=CrucibleRest.http_handlers_none_on_errors)
    ...
    def get_project(self, id):
        ...
        try:
            return self._request('/rest-service/projects-v1/%s?excludeAllowedReviewers=true' % id,
                                 http_handlers=CrucibleRest.http_handlers_none_on_errors)
        except StandardError:
            return None
    ...
    def is_script_update_available(self):
        return self._request('/rest/review-cli/1.0/version/updateCheck?version=%s' % SCRIPT_VERSION)[
            'isUpdateAvailable']
...
class PatchSource(object):
    ...
    def _validate_executable(self):
        try:
            ...
            Console.error('A %s repository was detected, but there was an error executing \'%s\': %s' % (
            self, self.configuration.executables[self.executable()], e))
            sys.exit(1)
    ...
    def matches_url(self, url):
        requested_path = urlparse.urlsplit(url.rstrip('/'))
        logging.debug('Matching remote url: %s', requested_path.__str__())
        for path in self.paths:
            local_path = urlparse.urlsplit(path.rstrip('/'))
...
class HgSource(PatchSource):
    ...
    def get_patch(self):
        diff_command = 'qdiff' if self.is_mq_active() else 'diff'
        diff_git = str(self.configuration.git_format)
        return check_output(
            [self.configuration.executables[self.executable()], diff_command, '--config', 'diff.git=' + diff_git])

    def is_mq_active(self):
        # Determine if there is a mq patch available
        # If there is no patch or if mq is not installed 'hg qtop' will raise an exception
        try:
            check_output([self.configuration.executables[self.executable()], 'qtop'])
            return True
...
class P4Source(PatchSource):
    ...
    def get_delete(self, file, rev):
        lines = check_output(
            [self.configuration.executables[self.executable()], 'print', '-q', file + '#' + rev]).splitlines()
    ...
    def get_patch(self):
        ...
        for line in opened:
            ...
            if match:
                ...
                # to be kept consistent with P4NewDiffHeaderParser
                if file_type in ['binary', 'xbinary', 'ubinary', 'uxbinary', 'tempobj', 'ctempobj', 'xtempobj',
                                 'symlink']:
                    logging.debug('Omitting binary file {0}'.format(location))
                    continue

                if change in ['add', 'branch']:
                    # 'branch' changes come from files being copied as a part of branch creation
                    # these don't get included in p4 diff, so treat them as adds here to include them in the patch
                    output += '\n' + self.get_add(location, rev)
                elif change == 'delete':
                    output += '\n' + self.get_delete(location, rev)
                else:
                    output += check_output([p4, '-z', 'tag', 'diff', '-du', location])
    ...
    def load_paths(self):
        ...
        for where in where_output:
            ...
            if len(where_split) == 3 and not where_split[0].startswith('-') and where_split[1].startswith(
                            '//%s' % self.p4info['Client name']):
    ...
    def matches_repository(self, repository_data):
        ...
        if self.p4info.get('Server address') == remote_address or self.p4info.get(
                'P4Sandbox broker port') == remote_address:
            for path in self.paths:
...
def main():
    ...
    if not args.patch:
        configuration = configuration.fill_interactively()
    ...
    if not args.patch:
        ...
        if rest.is_script_update_available():
            print('An updated version of this script is available. Visit %s to download it' % (
            configuration.url + '/plugins/servlet/viewReviewCLI'))

    # get a patch
    patch, source = PatchSource(configuration).load_patch()
    if not patch:
        Console.error(
            'Failed to get a patch%s. Please make sure to call the script from an SCM directory with local changes, or pipe in some input.'
    ...
    if not configuration.repository and not configuration.no_anchor:
        ...
        if detected_repository:
            ...
            print(
                'Detected Crucible repository %s. Use --repository or --noanchor options to override' % configuration.repository)
        else:
